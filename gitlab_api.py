...
async def get_merge_request_reviews(gitlab_url, project_id, access_token, mr_iid):
    discussions_url = f"{gitlab_url}/api/v4/projects/{project_id}/merge_requests/{mr_iid}/discussions"
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(discussions_url, headers=headers) as discussions_response:
            discussions = await discussions_response.json() if discussions_response.status == 200 else None
        ...
        async with session.get(approvals_url, headers=headers) as approvals_response:
            approvals = await approvals_response.json() if approvals_response.status == 200 else None
...
async def get_branch_merge_requests(gitlab_url, project_id, access_token, params):
    url = f"{gitlab_url}/api/v4/projects/{project_id}/merge_requests"
    headers = {"Private-Token": access_token, "Content-Type": "application/json"}
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers, params=params) as response:
