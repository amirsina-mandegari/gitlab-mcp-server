from gitlab_api import get_merge_request_details
...
async def get_merge_request_details(gitlab_url, project_id, access_token, args):
    ...
    status, mr, error_text = await get_merge_request_details(gitlab_url, project_id, access_token, mr_iid)
    if status == 200:
        result = f"# MR !{mr['iid']}: {mr['title']}\n\n"
        result += f"**Author**: {mr['author']['name']} (@{mr['author']['username']})\n"
        result += f"**Source**: `{mr['source_branch']}` → `{mr['target_branch']}`\n"
        ...
        if mr.get('pipeline'):
            ...
            result += f"**Pipeline**: {pipeline['status']} ({pipeline['web_url']})\n"
