from gitlab_api import get_branch_merge_requests
...
async def get_branch_merge_requests(gitlab_url, project_id, access_token, args):
    ...
    status, mrs, error_text = await get_branch_merge_requests(gitlab_url, project_id, access_token, params)
    if status == 200:
        if not mrs:
            return CallToolResult(
                content=[TextContent(type="text", text=f"No merge requests found for branch '{branch_name}'")]
            )
